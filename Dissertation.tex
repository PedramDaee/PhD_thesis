...
\begin{figure}
	...
	\caption{Plate notation of the intent modelling from feedback on recommended documents and keywords. The feature transformations are not shown. The feature for the $i^{th}$ keyword (shown as $\bm{x}_i$) is the $i^{th}$ row of the keyword-document matrix $\bm{X}$, and for the $j^{th}$ document (shown as $\bm{x}_j$) is the $j^{th}$ row of $\hat{\bm{X}}\tp \bm{X}$.}
