...
%&= \exp \left ( -2\sigma^{-2}\bm{w}\tp\bX\tp\by + \bm{w}\tp\Sigma^{-1}\bm{w} \right)
\end{align}    
 
\noindent where we dropped $p(\bD)$ and all the other terms which where constant with respect to $\bw$. The terms in Equation \ref{Eq:lin_rel_simple_derivation} are a quadratic function of $\bm{w}$, which has the same form as a multivariate Gaussian distribution in \ref{Eq:multi_Gauss}. This relation shows that the posterior should be multivariate Gaussian and its parameters can be found by matching the terms of \ref{Eq:lin_rel_simple_derivation} and \ref{Eq:multi_Gauss}. Therefore, $\Sigma^{-1} = ( \sigma^{-2}\bX\tp\bX + \tau^{-2} \eye ) $ and $\Sigma^{-1}\mu = \sigma^{-2}\bX\tp\by$. Therefore, the posterior of our simple linear regression can be described as 

\begin{align}\label{Eq:lin_rel_simple_posterior}
	p(\bm{w} \mid \bD) &= \normalpdf(\bw \mid \mu,\Sigma),\\
	\Sigma^{-1} &= (\sigma^{-2}\bX\tp\bX + \tau^{-2} \eye ), \nonumber\\
	\mu &= \Sigma\sigma^{-2}\bX\tp\by. \nonumber
\end{align}

Comparing the mean of the posterior, i.e. $\hat{\bw}_{MAP} = (\bX\tp\bX + \frac{\sigma^{2}}{\tau^{2}} \eye )^{-1}(\bX\tp\by)$, as a point estimate of the posterior distribution with the frequentist solution $\hat{\bw} = (\bX \tp \bX)^{-1}\bX\tp \by$ indicates that the two estimate are very similar with the only difference that $\tau^{-2} \eye$ acts as an added  in the covariance of posterior. This term is known as a regularization and has a relationship with the type of prior that was used in the modelling.

This simple Bayesian regression (Equation \ref{Eq:lin_rel_simple_posterior}) is actually used as the underlying data model in Publication I. However, ....

TO DO: revise the last paragraph and talk about regularization and the role of posterior. Then talk about posterior inference and its difficulties. Then finish with a few models.
