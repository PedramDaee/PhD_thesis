...
\noindent and provides as feedback the resulting posterior  $p(\gamma^j=1 \mid \bD, f^{j})$. The latent feedback likelihood can be represented as $p(f^{j}\mid \gamma^j) = \pi^j \gamma^j + (1-\pi^j) (1-\gamma^j)$, with $\pi^j$ being the likelihood for for latent feedback when $\gamma^j=1$. By doing the reverse of the Bayesian update in \ref{Eq:Bayesian_update_biased_fb}, we can infer $\pi^j$ as:

\begin{equation}\label{Eq:latent_knowledge_infer}
\pi^j \propto \dfrac{p(\gamma^j=1 \mid \bD, f^{j})}{p(\gamma^j =1\mid \bD)}.
\end{equation}  

We can then use the inferred user feedback $p(f^{j}\mid \gamma^j)$, instead of the observed one, to update the posterior for the parameters of interest.
...
Potential user bias to information revealed during the interaction has rarely been investigated. \cite{bias_warning} proposed a framework for measurement of such biases in visual analytics tools and investigated how to overcome it through informing the user of potential biases. Our work is different in a sense that we do not interrupt the interaction of the user but rather handle the potential biases by having a user model that accounts for them.  
%the model fitting to idiosyncrasies in the training data and hence not generalizing to new data

Overfitting has been investigated. Revealing the training data to the user increases the chance of overfitting.  
