...
The model proposed model is very simple as the noise of feedback is assumed to be known and the only unknown parameter is the intent vector $\bw$. As mentioned, we are interested to consider feedbacks from implicit sources such as neurophysiologic signals gathered from brain activities. The implicit feedback is extremely cheap, as the it requires no physical effort from the user, but at the same time extremely noisy. Our modelling solution is to use the automatic relevance detection model proposed in \ref{Eq:Bayes_rule_ARD} to endow the feedback likelihood to handle the inherent noise in the implicit feedback (here only on keywords, but straightforward to extend) when used in combination of the explicit feedbacks (here only on documents but straightforward to extend). The resulting model is depicted in Figure \ref{fig:Plate_intent_imp}.  

\begin{figure}
	\centering
	\includegraphics[width=0.80\linewidth]{Figures/Plate_diagram_intent_imp.pdf}
	\caption{Plate notation of the intent modelling from feedback on recommended documents and noisy, implicit feedback on keywords. The data model is based on \ref{Eq:Bayes_rule_ARD} with the feedbacks as the observational data.}
...
Intent modelling for article search has been studied in CITES. The user interface proposed in CITE considers a radial space for feedback... document retrieval was modelled differently... only based on user feedback on keywords.
Has been used also for image retrieval maybe cite a few.
