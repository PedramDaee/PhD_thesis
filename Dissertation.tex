...
\\\\
The first task is the expert knowledge elicitation for high-dimensional prediction. Experts in the fields usually have information beyond training data which can help to improve the prediction performance. The thesis contributes by proposing user models, as priors and likelihood functions, to directly connect expert knowledge about the relevance of parameters to a linear model responsible for prediction. The user model can account for complex user behaviour such as users updating their knowledge during the interaction. %, which allows to gain more information from interaction. 
