...
This chapter studies two general family of query algorithms for the interaction. Section \ref{AL_and_ED} reviews the methods that aim to query the most informative question from the user to maximize the predictive performance of the probabilistic model. Section \ref{MAP_and_BO} reviews the methods that are used to find the most relevant (rewarding) item with minimum number of interactions. 
...
\noindent where $\tilde{y}$ is the unobserved response variable,  $\tilde{\bm{x}}$ is the corresponding feature vector, and $\bF$ contains the gathered feedback till now. The user and data related parameters ($\phi_D$ and $\phi_f$) are omitted for brevity. The goal is to query the user about the feature, let's say $j^*$, that the corresponding feedback, i.e., $f_{j^*}$, would maximally improve this predictive distribution. The improvement can be measured by comparing the posterior predictive distribution after and before observing the feedback using $\KLt$-divergence, i.e., $\KL{p(\tilde{y} \mid \tilde{\bm{x}}, \bD,\bF,f_{j^*})} {p(\tilde{y} \mid, \tilde{\bm{x}}, \bD,\bF)}$. This utility function is known as the information gain and it measures the impact the new feedback on the posterior predictive distribution of the data. There are two issues with the information gain: first, we do not have access to the unobserved data $(\tilde{\bm{x}},\tilde{y})$, and second, we would only observe the user feedback $f_{j^*}$ after querying it from the user. Rather than computing the utility on the unobserved data, we use the available training data set to compute the utility. Unobserved feedback can be handled by computing the expected feedback that the user may provide after the query. The expectation would be taken with respect to the posterior predictive of user feedback (the distribution that the system believes the feedback would be generated from), i.e., $p(\tilde{f}_j \mid \bD,\bF)= \int_{\theta} p(\tilde{f}_j \mid \theta)p(\theta \mid \bD, \bF)d\theta$. The best query can then be  selected as
...
This utility (known as expected information gain) is expensive as it requires the computation of the posterior predictive distribution before and after the new feedback (which itself requires the computation of the corresponding posterior and the integral in Equation \ref{Eq:post_pred_data_fb}), computation of the $\KLt$-divergence, and computation of the posterior predictive of the feedback necessary for deriving the expectation. Publications II and III used expected information gain to extract expert's knowledge. TODO: say something about computation (EP as discussed before) and also about approximating the posterior predictive of data by Gaussian and therefore having analytical solution for KL, and then approximating the posterior predictive of feedback by Guassian (for feedback 1) and Bernoulli for other types...    
