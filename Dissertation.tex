...
Consider the problem of finding the argument that maximizes an objective function, i.e., $x_{j^*} = \argmax_{x_{j} \in \mathcal{X}} f(x_{j})$, in the design space $\mathcal{X}$. Now consider that the function of interest $f(x)$ is unknown  and the only way to gain information about it is to sequentially query it, i.e., ask the function value about a point of interest, such as $x_q \in \mathcal{X}$, and observe the corresponding function value $f(x_q)$ (or alternatively with some added noise). The natural goal for this black box optimization problem could be to find $x^*$ with minimum number of queries. This problem has been extensively investigated in the multi-armed bandit \cite{bubeck2012regret} and Bayesian optimization \cite{brochu2010tutorial,BO_review} literatures. A core characteristic of these approaches is that they need to make a compromise between asking queries that would provide new information about the hidden objective function (for example in areas that have not been explored), versus exploiting the current guess about where the maximum and querying it. This is known as the exploration exploitation trade-off and methods in the literature propose different acquisition function to balance it. Bayesian optimization and multi-armed bandits methods have been used in many applications such as clinical trials (finding the best treatment of a patient out of several alternatives) \cite{bubeck2012regret}, ad placement (finding the ad that would have the highest user click chance) \cite{Bandit_IR}, Automatic machine learning (searching in space of machine learning models) \cite{hoffman2014correlation}, reinforcement learning (finding the best action of an agent) \cite{brochu2010tutorial}, and personalized search systems \cite{ruotsalo2015interactive}. 

A focused application in this thesis is the personalize search system, where the goal is to find the most relevant item for a user by minimum interaction. As the user relevance profile over items is unobserved and only can be queried through recommendations and observing user feedback, one can can map this problem to the black-box optimization problem introduced above. In particular, the problem can be formulated as finding (recommending) the item $\bm{x}_{j^*}$ item that has the highest expected relevance (or equivalently highest expected feedback value). As explained in \ref{feedback_intent_modelling}, we consider the cases where the expected relevance of items have linear relationship with the unknown parameter $\bm{\theta}$\footnote{Note that we have changed the notation for the unknown weight vectors $\bm{w}$ in Section \ref{prob_model_user} to $\bm{\theta}$ to increase readability.}: 

\begin{equation}\label{Eq:black_box_optimization}
\bm{x}_{j^*} = \argmax_{\bm{x}_j \in \mathcal{X}} \E [f^j \mid \bm{x}_j] = \argmax_{\bm{x}_j \in \mathcal{X}} \bm{x}_j\tp \bm{\theta}.
\end{equation}

Given the history of interaction at time $t$, $\bF = \{(\bm{x}_i , f^i)\}_{i=1}^{t}$, the loss of the query algorithm (also known as acquisition function) is measured by the cumulative regret defined as:

\begin{equation}\label{Eq:regret}
regret_t = t \bm{x}_{j^*}\tp \bm{\theta} - \sum_{i=1}^{t} \bm{x}_j\tp \bm{\theta}.
\end{equation} 

Note that the cumulative regret is minimized if the most relevant item is recommended to the user. In the literature, there are m 


TODO: try to explain the regret a bit better and refer to a right paper (it is now a bit hand waving). Then go with the acquisition function and introduce Thompson sampling in more details. Then link to publications. 
...
The thesis can be summarized as attempts to model the user in some cases of human-in-the-loop machine learning scenarios. We believe that the this area would be of great importance in the future as the machine learning methods would always need the interaction for the user (for example for decision making) or collaboration. User modelling, can provide the machine with a model of how users perceive the world, and knowing that the interaction between the human and machine can be improved. In a following work \cite{peltola2018machine}, we showed that the personalized search systems can be improved if the user model acknowledges that the user may actually model the machine and exploit it in the interaction...
