...
\begin{itemize}
	\item We can keep this empty and start from next. Alternatively we can write some stuff here and have only one section as "Contributions and organization of the thesis". See which one works better for you
	%\item TODO: To get ideas for presentation of the story, look at Sami's coffee seminar slides from 4th of Feb 2019.
\end{itemize}
...
\begin{itemize}
	\item Thinking back, I think my main motivation should be on how to tackled limited interaction problem. In iui2016 we did it by adding new likelihoods, in JASIS by implicit interaction, in ML and IUI 18 by assuming strong priors.
	\item A) Small number of data ->add users
	\item B) user is the source of data
	\item Limited interaction: 1) need to be smart about what question to ask from the user 2) Should be able to consider all potential sources of feedback on different components of the interaction framework.
\end{itemize}
...
\begin{itemize}
	\item Explain general Bayesian modeling (posterior, likelihood, prior). represent uncertainty using probability distribution and then using Bayesian theorem for the inference, finding the distribution of the parameters of interest (unknown parameters), giving the observed variables and model assumptions.
...
\begin{itemize}
	\item Defining appropriate likelihood and defining appropriate priors
...
TODO: maybe start from this section
\section{Modelling the user}

\begin{itemize}
	\item Our general approach is to consider new likelihood function for the user feedback and put appropriate priors on top of that. Explain the feedback likelihoods?
\end{itemize}
...
\begin{itemize}
	\item Here have the same figure, explain how user feedback is gathered. explain that we consider two sets of works. One where there is external data other than user feedback and one where the user feedback IS the data. Then explain our interaction goals and acquisition functions and experimental design.
\end{itemize}

\chapter{Applications and results OR Summary of the Publications }

\begin{itemize}
	\item Go through the papers and tasks. 
\end{itemize}
...
\begin{itemize}
	\item Motivate future works such as next level of user modelling. Similar to overfitting work, but assuming a more active version of the user -> ATOM
\end{itemize}
