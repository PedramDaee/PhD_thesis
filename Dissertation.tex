...
%This would be easy to address since the application is trivial and not necessary explain.
In Publication II and III we target the case where the user has knowledge about the model parameters and is able to provide feedback regarding that if queried... EXPLAIN THE USER AND TASK... We consider the linear regression model with sparsity-inducing spike-and-slab prior on coefficients introduced in Equation \ref{Eq:ss_Bayesian_regression} as the underlying data model. The type of feedback naturally depends on the task and availability of user knowledge. Studies have shown that the type of domain knowledge in prediction tasks can be categprized in a small set \cite{concept_driven_CHI2019}. We consider three simple and natural types of feedback models. 
...
\begin{itemize}
	...
	\end{equation}
	\noindent where $\omega^2$ models the variance of error in user feedback. This type of feedback would be the least likely knowledge that a user may have. But still, in some specific applications, knowledge about value of coefficient may be available through related literature (for example [in some fields they report log(wj)?]) 
	...
	f_{j}^{rel} \sim \gamma_j \bernoullipdf(\pi^{rel}) + (1 - \gamma_j) \bernoullipdf(1 - \pi^{rel}).
	\end{equation}
	\noindent where $\pi^{rel}$ indicates the probability that user is correct about the feedback. This relevant/not-relevant feedback is the simplest form of knowledge that a user (domain expert for example) may have about a regression model. 
	...
	f_{j}^{dir} \sim I(w^j \geq 0) \bernoullipdf(\pi^{dir}) + I(w^j < 0) \bernoullipdf(1 - \pi^{dir}).
	\end{equation}
	\noindent where $\pi^{dir}$ indicates the probability that user is correct about the feedback.
\end{itemize}

The connection of the three mentioned feedback with model (Equation \ref{Eq:ss_Bayesian_regression}) is shown as a plate diagram in Figure XXX. The posterior of the unknown parameters given data observations and the user feedback can be written as XXXx

It should be noted that there is no restriction on the amount of feedback (or the types) available for computation of the posterior. The posterior computation is discussed in Section \ref{posteriro_inf}.
